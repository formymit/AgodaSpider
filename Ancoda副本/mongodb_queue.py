...
class MongoQueue():
    ...
    def agoda_push(self, url, country, province):
        try:
            self.db.insert({'_id': url,'country':country, 'province':province, 'status': self.OUTSTANDING})
            print(url, "插入队列成功")
        except errors.DuplicateKeyError as e: #报错则代表已经存在于队列中
            print(url, "已经存在与队列中")
            pass
    ...
    def pop_country(self, url):
        record = self.db.find_one({'_id':url})
        return record['country']

    def pop_province(self, url):
        record = self.db.find_one({'_id': url})
